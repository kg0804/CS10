/*

Program: Assignment12.java          Date: 04/25/2022


Author: Karan Gill
School: CHHS
Course: Computer Science 10
 

*/


package Assignments2;

import java.util.Scanner;

public class Assignment12
{


public static void main(String[] args)
{
	Scanner userinput = new Scanner(System.in);  // Create a Scanner object
	System.out.println("Enter a grade"); //user enters number
	int grade = userinput.nextInt(); //grab and store number

if ( grade < 50) // print grade if less then 50
{
	System.out.println("Your grade is F");
}

else switch (grade)  //print letter grade based on user input
{  
	  case 51:
	  case 52:
	  case 53:
	  case 54:
	    System.out.println("Your grade is D"); //print letter grade based on user input
	    break; 
	  case 55:
	  case 56:
	  case 57:
	  case 58:
	  case 59:
		System.out.println("Your grade is D+"); //print letter grade based on user input
		    break; 
	  case 60:
	  case 61:
	  case 62:
	  case 63:
	  case 64:
		System.out.println("Your grade is C"); //print letter grade based on user input
		    break;
	  case 65:
	  case 66:
	  case 67:
	  case 68:
	  case 69:
		System.out.println("Your grade is C+"); //print letter grade based on user input
		    break;
	  case 70:
	  case 71:
	  case 72:
	  case 73:
	  case 74:
		System.out.println("Your grade is B"); //print letter grade based on user input
		    break;
	  case 75:
	  case 76:
	  case 77:
	  case 78:
	  case 79:
		System.out.println("Your grade is B+"); //print letter grade based on user input
		    break;
	  case 80:
	  case 81:
	  case 82:
	  case 83:
	  case 84:
	  case 85:
	  case 86:
	  case 87:
	  case 88:
	  case 89:
		System.out.println("Your grade is A"); //print letter grade based on user input
		    break;
	  case 90:
	  case 91:
	  case 92:
	  case 93:
	  case 94:
	  case 95:
	  case 96:
	  case 97:
	  case 98:
	  case 99:
	  case 100:
		System.out.println("Your grade is A+"); //print letter grade based on user input
		    break; 

}

}
}

/* Screen Dump

Enter a grade
100
Your grade is A+



 */
